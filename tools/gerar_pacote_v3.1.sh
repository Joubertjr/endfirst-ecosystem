...
# Script para gerar PACOTE_EXECUCAO_COMPLETA_END_FIRST_v3.1.zip
# com MANIFEST.json consistente e incluído no ZIP
...
ZIP_NAME="PACOTE_EXECUCAO_COMPLETA_END_FIRST_v3.1.zip"
TEMP_DIR=$(mktemp -d)

echo "Gerando pacote ZIP v3.1..."
...
# Criar ZIP primeiro (sem MANIFEST ainda)
echo "Criando ZIP inicial..."
cd "$TEMP_DIR"
...
echo "Adicionando MANIFEST.json ao ZIP..."
cd "$REPO_ROOT"
zip "$REPO_ROOT/OUTPUTS/$ZIP_NAME" MANIFEST.json > /dev/null

# Atualizar MANIFEST.json para incluir ele mesmo
echo "Atualizando MANIFEST.json com hash próprio..."
python3 <<PYTHON_SCRIPT
import json
import hashlib
import zipfile
...
# Ler MANIFEST atual
with open("$REPO_ROOT/MANIFEST.json", "r") as f:
    manifest = json.load(f)

# Calcular hash do MANIFEST.json
with open("$REPO_ROOT/MANIFEST.json", "rb") as f:
    manifest_content = f.read()
    manifest_sha256 = hashlib.sha256(manifest_content).hexdigest()

# Adicionar MANIFEST.json ao manifest
manifest["arquivos"]["MANIFEST.json"] = manifest_sha256

# Salvar MANIFEST.json atualizado
with open("$REPO_ROOT/MANIFEST.json", "w") as f:
    json.dump(manifest, f, indent=2)

print(f"✅ MANIFEST.json atualizado com {len(manifest['arquivos'])} arquivos (incluindo MANIFEST.json)")
PYTHON_SCRIPT

# Atualizar ZIP com MANIFEST.json atualizado
echo "Atualizando ZIP com MANIFEST.json final..."
cd "$REPO_ROOT"
zip -f "$REPO_ROOT/OUTPUTS/$ZIP_NAME" MANIFEST.json > /dev/null

# Limpar diretório temporário
rm -rf "$TEMP_DIR"

echo ""
echo "✅ Pacote ZIP v3.1 gerado: OUTPUTS/$ZIP_NAME"
echo "✅ MANIFEST.json gerado: MANIFEST.json"
echo "✅ Commit final: $COMMIT_FINAL"
echo "✅ Total de arquivos no pacote: $(python3 -c "import json; f=open('MANIFEST.json'); m=json.load(f); print(len(m['arquivos']))")"
